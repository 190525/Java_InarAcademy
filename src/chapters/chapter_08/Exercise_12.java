...
public class Exercise_12 {
    public static void main(String[] args) {
        ...
        System.out.println(" Enter the filing status: (0-single filer, 1-married jointly or qualifying widow(er)," +
                " 2-married separately, 3-head of household)");
        int status = input.nextInt();
        if(!(status<=3&&status>=0)){
            System.out.println("Entered valid status is invalid");
            System.exit(0);
        }
        System.out.println("Please enter income: ");
        double income= input.nextDouble();
        System.out.printf("Tax is $%.3f", calculateTax(income, brackets[status], rates));


    }
   public static double calculateTax(double income, int[] bracket, double[] rates) {
        double tax = 0;
        if (income <= bracket[0]) {
            tax = income * rates[0];
